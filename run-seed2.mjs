...
const r2 = await db.insert(stores).values({ accountId: storeAccId1, name: "アロマサロン LUXE", description: "高級感あふれる完全個室のメンズエステサロンです。", address: "東京都渋谷区道玄坂1-2-3 LUXEビル5F", prefecture: "東京都", city: "渋谷区", phone: "03-1234-5678", access: "渋谷駅徒歩5分", openHours: "11:00", closeHours: "23:00", regularHoliday: "不定休", isPublic: true, reviewAvg: "4.80", reviewCount: 42 });
const storeId1 = r2[0].insertId;

const r3 = await db.insert(storeAccounts).values({ email: "store2@example.com", passwordHash: hash });
const storeAccId2 = r3[0].insertId;
const r4 = await db.insert(stores).values({ accountId: storeAccId2, name: "プレミアムエステ BLANC", description: "白を基調とした清潔感あるサロン。", address: "東京都新宿区歌舞伎町2-3-4 BLANCビル3F", prefecture: "東京都", city: "新宿区", phone: "03-9876-5432", access: "新宿駅東口徒歩7分", openHours: "12:00", closeHours: "24:00", regularHoliday: "月曜定休", isPublic: true, reviewAvg: "4.60", reviewCount: 28 });
const storeId2 = r4[0].insertId;

const ta1 = await db.insert(therapistAccounts).values({ email: "therapist1@example.com", passwordHash: hash });
const tAccId1 = ta1[0].insertId;
const t1 = await db.insert(therapists).values({ accountId: tAccId1, storeId: storeId1, displayName: "葵（あおい）", age: 24, height: 162, bust: 85, waist: 58, hip: 87, style: "スレンダー", catchphrase: "心も体も癒します♪", selfIntro: "初めまして！葵です。", isPublic: true, nominationCount: 128, reviewAvg: "4.90", reviewCount: 38 });
const tId1 = t1[0].insertId;

const ta2 = await db.insert(therapistAccounts).values({ email: "therapist2@example.com", passwordHash: hash });
const tAccId2 = ta2[0].insertId;
const t2 = await db.insert(therapists).values({ accountId: tAccId2, storeId: storeId1, displayName: "桜（さくら）", age: 22, height: 158, bust: 83, waist: 56, hip: 85, style: "かわいい系", catchphrase: "笑顔でお迎えします！", selfIntro: "こんにちは！桜です。", isPublic: true, nominationCount: 95, reviewAvg: "4.75", reviewCount: 29 });
const tId2 = t2[0].insertId;

const ta3 = await db.insert(therapistAccounts).values({ email: "therapist3@example.com", passwordHash: hash });
const tAccId3 = ta3[0].insertId;
const t3 = await db.insert(therapists).values({ accountId: tAccId3, storeId: storeId2, displayName: "凛（りん）", age: 26, height: 165, bust: 88, waist: 60, hip: 90, style: "スタイル抜群", catchphrase: "上質なひとときを", selfIntro: "凛と申します。", isPublic: true, nominationCount: 67, reviewAvg: "4.85", reviewCount: 21 });
const tId3 = t3[0].insertId;

const ca1 = await db.insert(customerAccounts).values({ email: "customer1@example.com", passwordHash: hash });
const cAccId1 = ca1[0].insertId;
await db.insert(customerProfiles).values({ accountId: cAccId1, nickname: "田中 太郎", memberLevel: 3, totalSpent: 85000 });

const ca2 = await db.insert(customerAccounts).values({ email: "customer2@example.com", passwordHash: hash });
const cAccId2 = ca2[0].insertId;
...
console.log("Seed complete! storeId1:", storeId1, "storeId2:", storeId2, "tId1:", tId1, "tId2:", tId2, "tId3:", tId3);
await conn.end();
