...
import { ChevronLeft, ChevronRight, Clock, Home, MessageCircle, Plus, PlusSquare, User } from "lucide-react";
  ...
  return (
    <AromaLayout title="出勤管理" showBack backHref="/therapist/dashboard" showNav navItems={navItems}>
      ...
      <div className="px-4 py-2 flex items-center justify-between bg-white border-y border-border/50">
        <button onClick={() => setMonthDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))} className="p-2 rounded-full active:bg-muted" aria-label="前月">
        ...
        <button onClick={() => setMonthDate(d => new Date(d.getFullYear(), d.getMonth() + 1, 1))} className="p-2 rounded-full active:bg-muted" aria-label="翌月">
      ...
      <div className="px-4 space-y-3">
        {list.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">この月の出勤予定はありません</p>
          </div>
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-sm rounded-2xl">
          <DialogHeader><DialogTitle>出勤申請</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div>
              <Label>日付</Label>
              <Input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className="mt-1 rounded-xl" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label>開始時間</Label>
                <Input type="time" value={form.startTime} onChange={e => setForm(f => ({ ...f, startTime: e.target.value }))} className="mt-1 rounded-xl" />
              </div>
              <div>
                <Label>終了時間</Label>
                <Input type="time" value={form.endTime} onChange={e => setForm(f => ({ ...f, endTime: e.target.value }))} className="mt-1 rounded-xl" />
              </div>
            </div>
            <Button className="w-full rounded-xl gradient-luxury text-white" onClick={() => createMut.mutate(form)} disabled={createMut.isPending}>
              {createMut.isPending ? "送信中..." : "申請する"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
