...
import { BookOpen, Camera, Grid3X3, Heart, Image, Plus, Trash2, Video, X } from "lucide-react";
  ...
  return (
    <AromaLayout showBack backHref="/therapist/dashboard">
      <div className="px-4 pt-4 pb-3 bg-white border-b border-border/30">
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-base text-foreground mb-1 truncate">{p?.displayName ?? "セラピスト名"}</div>
            ...
            {p?.catchphrase && <p className="text-xs text-muted-foreground line-clamp-2">{p.catchphrase}</p>}
          </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        {tab === "grid" ? (
          <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 gap-0.5 bg-border/20 pb-24">
            {list.length === 0 ? (
              ...
              >
                {post.imageUrl ? (
                  ...
                  <div className={`w-full h-full flex items-center justify-center text-xs font-bold ${post.postType === "attendance" ? "bg-teal-50 text-teal-700" : "bg-purple-50 text-purple-700"}`}>
                    {post.postType === "attendance" ? "出勤" : "日記"}
                  </div>
                )}
              </motion.button>
            ))}
          ...
          <motion.div key="diary" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="px-4 py-3 space-y-3 pb-24">
            {diaryPosts.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-16 text-center">
                <BookOpen className="w-10 h-10 text-muted-foreground/30" />
                <p className="text-sm text-muted-foreground">日記の投稿はありません</p>
              </div>
            ) : diaryPosts.map((post: any, i: number) => (
              <motion.div key={post.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-2xl p-4 shadow-luxury">
                <div className="flex items-start justify-between mb-2">
                  <span className="text-xs text-muted-foreground">
                    {new Date(post.createdAt).toLocaleDateString("ja-JP", { year: "numeric", month: "long", day: "numeric" })}
                  </span>
                  <button onClick={() => deleteMut.mutate({ id: post.id })} className="p-1 text-red-400 hover:text-red-600 transition-colors" aria-label="削除">
            ...
            >
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-muted-foreground">{new Date(selectedPost.createdAt).toLocaleDateString("ja-JP")}</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <button onClick={() => deleteMut.mutate({ id: selectedPost.id })} className="text-xs text-red-500 flex items-center gap-1">
        ...
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <div className="space-y-3">
            ...
            <Textarea
              ...
              rows={5}
              placeholder={form.postType === "attendance" ? "本日の出勤情報を入力してください" : "今日の出来事を書いてみましょう"}
              maxLength={500}
            />
            <input
              ref={fileInputRef}
...
function CalendarDot() {
  return <div className="w-5 h-5 rounded border-2 border-teal-700 border-t-4" />;
}
