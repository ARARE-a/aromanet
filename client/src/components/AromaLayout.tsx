...
interface AromaLayoutProps {
  ...
  title?: string;
  titleLogo?: boolean; // show AromaNet logo instead of text title
  showBack?: boolean;
  backHref?: string;
  showNav?: boolean;
  ...
  className?: string;
  noPadding?: boolean; // skip default padding-bottom
}
...
function NavItem({ href, icon, activeIcon, label }: { href: string; icon: React.ReactNode; activeIcon?: React.ReactNode; label: string }) {
  ...
  return (
    <Link href={href}>
      <div className={cn(
        "flex flex-col items-center justify-center gap-[2px] w-[60px] h-full cursor-pointer",
...
export function StatusBadge({ status }: { status: string }) {
  const configs: Record<string, { label: string; className: string }> = {
    ...
    waiting: { label: "待機中", className: "bg-sky-50 text-sky-700 border-sky-200" },
    ...
    change_requested: { label: "変更申請", className: "bg-orange-50 text-orange-700 border-orange-200" },
  };
  ...
  return (
    <span
      ...
      style={{
        ...
        color: level === 6 ? "#fff" : colors[level],
        backgroundColor: level === 6 ? "#1a1a1a" : `${colors[level]}20`
      }}
    >
